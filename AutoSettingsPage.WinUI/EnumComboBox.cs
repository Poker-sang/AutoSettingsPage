...
public sealed partial class EnumComboBox : ComboBox
{
    ...
    public EnumComboBox()
    {
        ...
        {
            if (sender is EnumComboBox { ItemsSource: IEnumerable<IReadOnlyStringPair<object>> enumerable } box)
    ...
    private static void OnSelectedEnumPropertyChanged(DependencyObject o, DependencyPropertyChangedEventArgs e)
    {
        if (o is EnumComboBox s)
            s.SelectedItem = (s.ItemsSource as IEnumerable<IReadOnlyStringPair<object>>)?.FirstOrDefault(r => Equals(r.Value, s.SelectedEnum));
    }
}
